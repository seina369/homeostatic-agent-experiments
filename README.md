...
「感情の粒度」の問いを調べるため、2026-09-09より、実際の(訓練していない)
...
Colab上でこれを実行するための手順一式。

**2026-09-12時点の進捗(1.5Bでの実測と、それを受けた確定)**:
Qwen2.5-1.5B-Instruct(fp16、Colab T4)で20エピソード(200応答)を実測した
(`torch_speed_probe_results.json`)。エピソード秒数は平均12.47±2.16秒、
正答率0.500(0.5Bの0.080から上昇し、error信号がエピソード内で実際に動く
ようになった)、書式不履行率0.125、平均トークン数/応答30.1、エントロピー
平均0.711(p10=0.363、p50=0.681、p90=1.124、最大2.454)。課題の種類別の
正答率は add 0.775(31/40)、mul 0.714(25/35)、sub 0.605(23/38)、
count 0.457(21/46)、reverse 0.000(0/41)。

この結果から、事前登録の追記欄「2026-09-12」に理由つきで記録した上で、
次を確定した: 課題から reverse を除外し add/sub/mul/count の4種に固定
(常に失敗する課題は、行動によって増減しない誤答を積むだけで損傷信号の
趣旨に合わない。「不可能な課題への反応」は別実験の候補として切り離した)。
B0を450→340、budget逸脱の閾値を112.5→85(閾値=B0の25%を平均的な
エピソードの8〜9課題目で下回る、という前回と同じ基準を1.5Bの平均応答長で
再計算)。エントロピーの最適値・安全域を U_OPT/MIN/MAX=1.0/0.0/3.0 →
0.7/0.0/2.5(実測の中央値と最大値に揃えた。U_OPTには物理的根拠がなく
設計者の選択である旨を明記)。温度1.0とプロンプトは据え置き。
標本は、対照条件C0を15 seed×30 episode/seedに確定。処置条件T1のseed数は
GRPOの速度実測後に決める(最低10)が、T1がn<15になった場合はC0側も
seed 0..n−1 だけを比較に使う、と先に決めて記録した。

**次の段階はC0(学習なし・推論のみの対照条件)の実行**。C0・T1の本実験は
まだ行っておらず、粒度スコアに関する結果は現時点では何もない。
...
- 言語モデル実験は、0.5B・1.5Bでの速度実測(各20エピソード)を終え、
  環境のパラメータと標本を確定した段階。C0・T1の本実験はまだ行っておらず、
  粒度スコアの結果はない。エントロピーの最適値(U_OPT)は実測の中央値に
  合わせた設計者の選択であり、逸脱の絶対値の解釈にはその恣意性が残る。
...
`llm_grounding/Kaggle_run_stage1.ipynb`(固定のノートブック。何を実行するかは `llm_grounding/run_config.json` で
指定し、Save & Run All で回す。保存先は `/kaggle/working/results`)。
...
「感情の粒度」の問いを調べるため、小型言語モデルへの接続実験を進めている。
